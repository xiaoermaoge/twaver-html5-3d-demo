var demo = {
	...
	init: function (htmlElementId) {
		...
		// new mono.QuickFinde 根据数据上的某个属性快速地查询出匹配的数据 获取样式属性 (client)
		...
		console.log('demo.typeFinder', demo.typeFinder);
		console.log('demo.labelFinder', demo.labelFinder);
		// 创建3D模型
		var camera = new mono.PerspectiveCamera(30, 1.5, 30, 30000);
		network.setCamera(camera);
		
		// 默认交互，提供了默认的交互能力，例如键盘/鼠标镜头漫游等
		...
		// 编辑交互，提供基于鼠标进行位置拖拽、角度拖拽、拉伸拖拽等综合编辑能力
		...
		// 重载network.isSelectable函数 允许场景中client的type为'rack'的机柜物体可以选中
	...
	handleDoubleClick: function (e, network) {
		var camera = network.getCamera();
		// 获得DefaultInteraction对象 默认交互，提供了默认的交互能力，例如键盘/鼠标镜头漫游等
